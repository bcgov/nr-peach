...
1. Run `npm approve-scripts --allow-scripts-pending` to list pending packages.
2. If the list is empty, report that no scripts are pending approval and exit.
3. For each pending package, run `npm view <package>@<version> scripts` to retrieve the actual script contents.
4. Analyze each script for security risks:
...
5. Present a clean console summary for each package:
...
6. For each package sequentially, invoke the user confirmation tool via `vscode_askQuestions` to present the options exactly as:
