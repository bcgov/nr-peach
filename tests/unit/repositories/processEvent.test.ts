...
import { ProcessEventRepository } from '#src/repositories/processEvent';
...
describe('ProcessEventRepository', () => {
  ...
  describe('prune', () => {
    it('should build a delete query for the given assetId', () => {
      const assetId = 42;
      const compiled = new ProcessEventRepository(mockDb).prune(assetId).compile();

      expect(getDefinedOperations(compiled.query)).toEqual(['kind', 'from', 'where']);
      expect(compiled.query.kind).toBe('DeleteQueryNode');
      expect(compiled.sql).toBe('delete from "pies"."process_event" where "asset_id" = $1');
