 ...
 */
export type ProcessEventSet = ProcessEventSet1 & ProcessEventSet2;
export type ProcessEventSet1 = Header;
/**
 * Represents an event concept. It allows either a date or a datetime, but not a mix of both.
 * One of the start properties is required, but the end properties are optional.
 */
...
export interface Header {
  ...
  /**
   * A valid NRIDS IT Service Management code which identifies the source
   * system, service or asset that the data originates from.
   */
  system_id: string;
  /**
...
export interface ProcessEventSet2 {
  process_event: [ProcessEvent, ...ProcessEvent[]];
}
/**
 * Represents a process concept at a specific event in time.
 */
...
export interface Event1 {
  ...
  end_date?: string;
  /**
   * The end datetime of the event in ISO 8601 format. This is optional if `start_datetime` is used.
   */
  end_datetime?: string;
}
/**
 * This schema is used when the event starts with a date (`start_date`),and optionally,
 * an end date (`end_date`) can be specified. Mixing a start date with a datetime is not allowed.
 */
...
/**
 * This schema is used when the event starts with a datetime (`start_datetime`), and optionally,
 * an end datetime (`end_datetime`) can be specified. Mixing a start datetime with a date is not allowed.
 */
...
export interface Coding {
  ...
  code_display?: string;
  /**
   * An ordered set of code symbols, where the last element must match the code attribute.
   * The set must contain at least one symbol, preserve order, and not include duplicates.
   */
...
export interface Process2 {
  /**
   * An optional description of the current condition or update of an application or authorization.
   * Additional details about the current state are frequently conveyed (e.g., 'Pending Review', 'Under Inspection').
   * Statuses may be defined by the line of business.
   */
  status?: string;
  /**
