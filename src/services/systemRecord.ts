...
export const assertSingleSystemRecordService = async (recordId: string): Promise<Selectable<PiesSystemRecord>> => {
  const systemRecords = await new SystemRecordRepository().findBy({ recordId }).execute();
...
export const findSingleSystemRecordService = async (
  ...
  if (systemId) {
    return new SystemRecordRepository().findBy({ recordId, systemId }).executeTakeFirstOrThrow();
  }
