...
import { cacheableRead, cacheableUpsert, findWhereOrUpsert, transactionWrapper } from './helpers/index.ts';
import {
...
export const addRecordLinkageService = async (data: RecordLinkage): Promise<void> => {
  await transactionWrapper(async (trx) => {
    ...
    let systemRecord = await new SystemRecordRepository(trx)
      .findWhere({ systemId: data.system_id, recordId: data.record_id })
      .executeTakeFirst();
    ...
    if (!systemRecord) {
      ...
      systemRecord = await findWhereOrUpsert(new SystemRecordRepository(trx), {
    ...
    let linkedSystemRecord = await new SystemRecordRepository(trx)
      .findWhere({ systemId: data.linked_system_id, recordId: data.linked_record_id })
      .executeTakeFirst();
    ...
    if (!linkedSystemRecord) {
      ...
      linkedSystemRecord = await findWhereOrUpsert(new SystemRecordRepository(trx), {
